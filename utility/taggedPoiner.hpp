...
namespace simp {
	...
	class PtrOrInt
	{
		static constexpr std::size_t nr_unused_lower_ptr_bits = std::bit_width(alignof(PointedAt)) - 1;
		...
		{
			constexpr std::uintptr_t tag_mask = (1ull << nr_unused_lower_ptr_bits) - 1;
	...
	public:
		...
		PtrOrInt(Int_T const val, TagEnum const tag) noexcept 
			:ptr(std::bit_cast<PointedAt*>(((std::uintptr_t)val << nr_unused_lower_ptr_bits) | (std::uintptr_t)tag)) 
		{ 
		...
		PtrOrInt(Int_T const val) noexcept requires (std::is_same_v<TagEnum, PtrState>)
			:ptr(std::bit_cast<PointedAt*>(((std::uintptr_t)val << nr_unused_lower_ptr_bits) | (std::uintptr_t)PtrState::integer))
		{}
		...
		Int_T get_int() const noexcept 
		{
			...
			const auto shifted = std::bit_cast<Intermediate>(this->ptr) >> nr_unused_lower_ptr_bits; //respect sign when shifting
